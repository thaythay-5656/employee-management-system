...
// ════════════════════════════════════════════════════════════════════════════
// BONUS FIX — src/components/layout/sidebar.tsx
//
// Your current sidebar has navByRole with 4 keys: admin, hr, manager, employee
// But your Django backend only has 3 roles: admin, manager, employee
// The "hr" key is never matched so the sidebar renders null for hr users.
//
// REPLACE your entire sidebar.tsx with this file.
// ════════════════════════════════════════════════════════════════════════════

import { Link, useRouterState } from "@tanstack/react-router";
import {
  LayoutDashboard, Users, Building2, CalendarCheck, Wallet,
  PalmtreeIcon, Megaphone, Settings, UserCircle, Sparkles,
  Briefcase, ShieldCheck, ScrollText,
} from "lucide-react";
import { useAuthStore } from "@/store/auth-store";
import { cn } from "@/lib/utils";
...
// ── Nav items per role — only 3 roles to match Django ────────────────────
const navByRole: Record<Role, NavItem[]> = {
  admin: [
    { to: "/dashboard",    label: "Dashboard",     icon: LayoutDashboard },
    { to: "/employees",    label: "Employees",      icon: Users           },
...
// Role label shown under the logo
  ...
  return (
    <aside className="flex h-full w-64 flex-col bg-sidebar border-r border-sidebar-border">
      ...
      <div className="px-4 py-4 border-t border-sidebar-border">
        <div className="rounded-lg bg-gradient-to-br from-primary/20 to-accent/10 p-3 border border-primary/20">
          <div className="text-xs font-medium text-sidebar-foreground">Need help?</div>
          <div className="text-[11px] text-muted-foreground mt-0.5">Check our HR handbook</div>
        </div>
      </div>
    </aside>
  );
