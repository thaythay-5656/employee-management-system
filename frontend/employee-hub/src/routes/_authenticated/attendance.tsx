...
import { exportToExcel, exportTableToPDF } from "@/lib/export";
import { toast } from "sonner";
import { type Attendance } from "@/api/services";
import { useAttendance, useAttendanceMutations, useEmployees } from "@/hooks/useEMS";
...
function AttendancePage() {
  ...
  const { createAttendance, patchAttendance } = useAttendanceMutations(refetch);
  ...
  // Scope attendance by role
  let scoped = attendance;
  if (isEmployee && myId) {
    scoped = attendance.filter((a) => a.employee === myId);
  } else if (isManager && me) {
    const teamIds = new Set(
      employees.filter((e) => e.department === me.department).map((e) => e.id)
    );
    scoped = attendance.filter((a) => teamIds.has(a.employee));
  }

  // Apply filters
  const filtered = scoped.filter((a) => {
    // Django field: attendance_date
  ...
  // Clock in / Clock out → creates an attendance record via the API
  const now = new Date();
  const todayStr = now.toISOString().slice(0, 10);
  const todayRec = pendingAttendance ?? scoped.find(
    (a) => a.employee === myId && a.attendance_date === todayStr
  );

  const clockIn = async () => {
    if (!myId) return toast.error("No employee profile linked");
    try {
      ...
      setPendingAttendance(createdAttendance);
  ...
  const clockOut = async () => {
    if (!todayRec?.id) return toast.error("No check-in found for today");
    if (todayRec.check_out) return toast.error("You have already checked out");

    try {
      const updatedAttendance = await patchAttendance(todayRec.id, {
        check_out: now.toTimeString().slice(0, 8),
        status: "present",
      }) as Attendance;
      setPendingAttendance(updatedAttendance);
      toast.success(`Checked out at ${now.toLocaleTimeString()}`);
    } catch {
  ...
  const onExportExcel = () => {
    exportToExcel(
      filtered.map((a) => ({
        ...
        Employee: getEmployeeName(a.employee),
        "Check In": a.check_in ?? "—",
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <PageHeader title="Attendance" description="Attendance log." />
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={
          isEmployee
            ? "Your recent check-ins and work hours."
            : isManager
            ? "Your team's attendance."
            : "Organization-wide attendance log."
        }
        actions={
          <div className="flex gap-2">
            {isEmployee && (
              <>
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={clockOut}
                  disabled={!todayRec || !!todayRec.check_out}
                >
        ...
        <div className="glass rounded-xl p-4 flex flex-col sm:flex-row gap-3">
          ...
          {!isManager && (
            <Select value={deptFilter} onValueChange={setDeptFilter}>
              ...
              <SelectContent>
                <SelectItem value="all">All departments</SelectItem>
                {[...new Set(employees.map((e) => e.department))].map((dId) => {
                  const emp = employees.find((e) => e.department === dId);
                  return (
          ...
          <table className="w-full text-sm">
            <thead className="text-xs uppercase text-muted-foreground bg-muted/30">
              <tr>
                ...
                {/* Django fields: check_in, check_out */}
                <th className="text-left px-4 py-3 font-medium">Check-in</th>
                <th className="text-left px-4 py-3 font-medium">Check-out</th>
                <th className="text-left px-4 py-3 font-medium">Status</th>
              </tr>
